...
const ItemsPageComponent = () => {
    ...
    return (
        <div className="itemsPage min-h-screen p-6 dark:bg-black bg-gradient-to-r from-sky-300 to-indigo-400 dark:from-gray-900 dark:to-gray-800 flex items-start justify-center relative overflow-hidden">
            {/* Background Elements */}
            <div className="absolute inset-0 bg-[url('/grid.svg')] opacity-10 animate-pulse"></div>
            <div className="absolute inset-0 bg-gradient-to-b from-blue-500/5 to-transparent"></div>

            {/* Main Container */}
            <div className="relative w-full max-w-[1000px] dark:bg-black/90 bg-white rounded-2xl border-2 border-blue-500/30 shadow-2xl p-6 backdrop-blur-xl">
                {/* Holographic Effects */}
                ...
                <div className="flex justify-between items-center mb-8">
                    <div className="space-y-1">
                        ...
                        <p className="text-sm dark:text-cyan-400/80 text-blue-500 font-mono">Item Management Interface</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {items.map((item) => (
                        <div
                            key={item.id}
                            className="group dark:bg-black/80 bg-white p-3 rounded-lg border-2 dark:border-blue-500/20 border-blue-500 dark:hover:border-cyan-400/50 hover:border-blue-500/50 cursor-pointer transition-all duration-300 relative"
                        >
                            {/* Hover Effect */}
                            ...
                            <div className="flex flex-col items-center space-y-3 relative z-10">
                                ...
                                flex items-center justify-center overflow-hidden border-4 dark:border-cyan-500/50 border-blue-500 shadow-xl">
                                    <img
                                        src={item.image}
                                        alt={item.title}
                                ...
                                {/* Item Title */}
                                <h2 className="text-lg font-bold text-transparent bg-clip-text dark:bg-gradient-to-r dark:from-cyan-300 dark:to-blue-400 bg-gradient-to-r from-blue-500 to-cyan-400 font-mono text-center">
                                    {item.title.toUpperCase()}
                                </h2>

                                {/* Item Description */}
                                <p className="dark:text-cyan-400/80 text-blue-500 text-xs font-mono text-center min-h-[60px]">
                                    {item.description}
                                </p>

                                {/* Owned Quantity and Use Button */}
                                <div className="w-full flex items-center justify-between mt-2">
                                    <div className="dark:bg-black/50 bg-transparent px-2 py-0.5 rounded-full border dark:border-cyan-500/30 border-blue-300">
                                        <span className="dark:text-cyan-300 text-blue-500 font-mono text-xs">
                                            OWNED: {item.owned}
                                        </span>
                                    </div>
                                    <button
